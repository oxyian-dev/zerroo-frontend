...
const EditSpecificationList = () => {
    ...
    useEffect(() => {
        setLoading(true)
        fetcher(`/api/item-specifications/list/${id}`)
            .then(r => r.json())
            .then(res => {
                setName(res.name)
    ...
    return loading ? (
        ...
        <Formik
            ...
            onSubmit={async (values, { setSubmitting }) => {
                setSubmitting(true)
                return await fetcher(`/api/item-specifications/list/${id}`,
                    { method: 'put', body: constructFormData(values) })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status === 'success') {
                            enqueueSnackbar('Specification List Edited Successfully', { variant: 'success' })
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Card variant="outlined">
                                <CardContent>
                                    <Typography variant="h2" textAlign="center">Edit Specification List</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
...
export default EditSpecificationList;