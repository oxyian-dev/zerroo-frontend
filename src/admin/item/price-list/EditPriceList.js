...
import { IconCoin, IconPercentage } from "@tabler/icons";
...
import { constructFormData, discount, nonull, round } from "../../../utils/util";
    ...
    return loading ? (
        ...
        <Formik
            validationSchema={Yup.object().shape({
                ...
                gst_percent: Yup.number().required('GST is Required'),
                pv: Yup.number().min(0).required('PV Required')
            })}
            onSubmit={async (values, { setSubmitting }) => {
                setSubmitting(true)
                return await fetcher(`/api/price-lists/${id}`,
                    { method: 'put', body: constructFormData(values) })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status === 'success') {
                            enqueueSnackbar('Price List Edited Successfully', { variant: 'success' })
            ...
            initialValues={nonull({
                ...values,
                discount: discount(values.mrp, values.price) || 0,
                margin: round(values.price - values.cost - (values.gst_percent / 100 * values.price) - values.pv, 2)
            })}>
            {({
                errors,
                handleBlur,
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Card variant="outlined">
                                <CardContent>
                                    <Typography variant="h2" textAlign="center">Edit Price List</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                        <Grid item xs={6}>
                        ...
                        <Grid item xs={4}>
                            <FormControl fullWidth error={Boolean(touched.gst_percent && errors.gst_percent)}>
                                ...
                                {touched.gst_percent && errors.gst_percent && (
                                    <FormHelperText error id="error-gst_percent">
                                        {errors.gst_percent}
                                    </FormHelperText>
                                )}
                            </FormControl>
