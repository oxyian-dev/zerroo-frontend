...
export default function EditComboGroup() {
  const { id } = useParams();
  ...
  return loading ? (
    ...
    <Formik
      ...
      onSubmit={async (values, { setSubmitting }) => {
        setSubmitting(true)
        return await fetcher(`/api/combos/groups/${id}`, {
          method: 'put',
          ...
          .then(res => {
            if (res.status === 'success') {
              enqueueSnackbar('Combo Group Edited Successfully', { variant: 'success' })
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card variant="outlined">
                <CardContent>
                  <Typography variant="h2" textAlign="center">Edit Combo Group</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={6}>
